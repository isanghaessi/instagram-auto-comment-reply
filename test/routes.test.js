...
test('authenticated session cookie can access dashboard', async () => {
  ...
  try {
    ...
    assert.match(html, /action="\/account\/token"/);
    assert.doesNotMatch(html, /\/auth\/instagram\/start/);
  } finally {
    db.close();
  }
});

test('POST /account/token verifies token, saves encrypted account, and redirects home', async () => {
  const { db } = createTestDb();
  ...
  const app = createServer({
    ...
    instagramClient: {
      async getAccount(token) {
        calls.push(['getAccount', token]);
        return { user_id: 'ig-user-from-token', username: 'creator', account_type: 'MEDIA_CREATOR' };
      }
    },
  ...
  try {
    ...
    const response = await request(app, '/account/token', {
      method: 'POST',
      headers: sameOriginHeaders({ Cookie: cookie, 'Content-Type': 'application/x-www-form-urlencoded' }),
      body: formBody({ accessToken: 'dashboard-long-lived-token' })
    });
    const account = getAccount(db);

    assert.equal(response.status, 302);
    assert.equal(response.headers.get('location'), '/');
    assert.deepEqual(calls, [['getAccount', 'dashboard-long-lived-token']]);
    assert.equal(account.instagram_user_id, 'ig-user-from-token');
    assert.equal(account.username, 'creator');
    assert.equal(account.account_type, 'MEDIA_CREATOR');
    assert.notEqual(account.access_token_encrypted, 'dashboard-long-lived-token');
    assert.equal(decryptText(account.access_token_encrypted, config.encryptionKey), 'dashboard-long-lived-token');
    assert.ok(Date.parse(account.token_expires_at) >= before + 59 * 24 * 60 * 60 * 1000);
    assert.ok(Date.parse(account.token_expires_at) <= Date.now() + 61 * 24 * 60 * 60 * 1000);
  } finally {
    db.close();
  }
});

test('POST /account/token renders generic failure without saving or leaking token', async () => {
  const { db } = createTestDb();
  const leakedToken = 'manual-token-that-must-not-render';
  const app = testApp({
    db,
    instagramClient: {
      async getAccount(token) {
  ...
  try {
    ...
    const response = await request(app, '/account/token', {
      method: 'POST',
      headers: sameOriginHeaders({ Cookie: cookie, 'Content-Type': 'application/x-www-form-urlencoded' }),
      body: formBody({ accessToken: leakedToken })
    });
    const html = await response.text();

    assert.equal(response.status, 502);
    assert.match(html, /Unable to connect Instagram/);
...
test('OAuth routes are not mounted in token connection mode', async () => {
  const { db } = createTestDb();
  const app = testApp({ db });
  ...
  try {
    ...
    const callbackResponse = await request(app, '/auth/instagram/callback?code=auth-code&state=state');

    assert.equal(startResponse.status, 404);
    assert.equal(callbackResponse.status, 404);
  } finally {
    db.close();
  }
});
...
test('unauthenticated POST actions redirect to login', async () => {
  ...
  const app = testApp({ db });
  const actions = [
    ['/account/token', formBody({ accessToken: 'token' })],
    ['/media/sync', formBody({})],
