...
Instagram Creator/Business 계정 1개의 long-lived access token을 Dashboard에 붙여넣어 연결한 뒤, 선택한 게시물의 댓글을 60초 주기로 확인하여 룰에 맞는 댓글 작성자에게 Instagram Private Reply(DM)를 자동 발송합니다.

## 주요 기능

- 관리자 비밀번호 기반 로컬/셀프호스팅 웹 UI
- Instagram long-lived access token 수동 연결
...
- Node.js 22+
- Node.js 서버 또는 HTTPS 리버스 프록시가 연결된 서버
...
토큰 방식에서는 OAuth callback URL이 필요 없습니다. 운영에서는 관리자 비밀번호와 access token 보호를 위해 HTTPS 뒤에서 실행하는 것을 권장합니다.

## Instagram Token Setup Checklist

1. Meta Developer App을 생성합니다.
2. Instagram API with Instagram Login을 활성화합니다.
3. `Instagram > API setup with Instagram business login` 화면에서 연결할 Instagram Professional(Creator/Business) 계정 옆 `Generate token`을 클릭합니다.
4. Instagram에 로그인하고 권한을 승인한 뒤 long-lived access token을 복사합니다.
5. 서버 Dashboard에 로그인해 `Access Token` 입력란에 토큰을 붙여넣고 저장합니다.
6. 서버가 `/me` 호출로 계정을 검증한 뒤 token을 암호화해 SQLite DB에 저장합니다.

## 사용 흐름

1. 서버를 실행하고 관리자 UI에 로그인합니다.
2. Dashboard에서 Meta App Dashboard로 발급받은 Instagram access token을 붙여넣어 계정을 연결합니다.
...
- `ADMIN_PASSWORD`는 16자 이상 랜덤 문자열을 권장합니다. 로그인 실패는 IP 기준으로 일정 횟수 이후 일시적으로 제한됩니다. 리버스 프록시 뒤에 배포할 경우 실제 client IP와 lockout 범위가 의도대로 잡히도록 신뢰할 프록시/네트워크 설정을 별도로 확인하세요.
...
- 댓글 좋아요는 token 연결 성공을 막지 않도록 별도 engagement scope를 요구하지 않는 보조 기능입니다. 앱/계정/콘텐츠 제약에 걸려 실패할 수 있으며, 실패해도 DM 성공은 유지되고 `Logs`에 좋아요 실패만 별도 기록됩니다.
